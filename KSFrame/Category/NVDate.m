...
- (id)init {
    if (self = [super init]) {
        ...
        //[_dateFormatter setDateFormat:@"yyyy年MM月dd日hh时mm分ss秒"];
...
+(NSString *)dayDescFromDate:(NSString *)string;
{
    ...
    NSDateComponents *comps = [calendar components:NSWeekdayCalendarUnit fromDate:inputDate];
    long week = [comps weekday]-1;//0-6 星期日到星期六
    NSString *weekStr = @"天一二三四五六";
    [inputFormatter setDateFormat:@"yyyy年MM月dd日"];
    return [NSString stringWithFormat:@"%@ 星期%C",[inputFormatter stringFromDate:inputDate],[weekStr characterAtIndex:week]];
}
